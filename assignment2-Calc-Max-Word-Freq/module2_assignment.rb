...
class LineAnalyzer
  ...
  def calculate_word_frequency
    word_freq = Hash.new(0)  # need to give a default value for hash
    word_array = @content.downcase.split # need to transfre to downcase
    ...
    word_freq.each do |word, freq|
      @highest_wf_words.push(word) if @highest_wf_count == freq
    end
  end
end
...
class Solution
  ...
  def analyze_file
    ...
    File.open(file_path).each do |line|
      each_analyzer = LineAnalyzer.new(line, count += 1)
      @analyzers.push(each_analyzer)
    end
  end
  ...
  def calculate_line_with_highest_frequency
    freq_array = Array.new
    @highest_count_words_across_lines = Array.new
    @analyzers.each do |analyzer|
      freq_array.push( analyzer.highest_wf_count)
    end
    @highest_count_across_lines = freq_array.max # know how to use '.max'
